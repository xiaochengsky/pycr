...
class EfficientNet_B4_RGA_GAP_Identity_SnapMix_CE(nn.Module):
    def __init__(self, cfg):
        super(EfficientNet_B4_RGA_GAP_Identity_SnapMix_CE, self).__init__()
        self.cfg = cfg
    ...
    def forward(self, inputs, targets=None, extract_features_flag=False, features_type="after", ya=None, yb=None, lam_a=None, lam_b=None, reback=None):
        ...
        if self.layer:
            c_features = self.layer(b_features)

        if extract_features_flag and (features_type == 'backbone'):
            # [N, C, H, W]
            return b_features

        features = self.aggregation(c_features)
        ...
        if self.training:
            ...
            if reback == "eval_cls_backbone":
                return pred_logit, c_features.detach()
            elif reback is None:
                return pred_logit
