# -*- coding: utf-8 -*-
# @Time: 2023/6/1 下午5:52
# @Author: YANG.C
# @File: mobileone.py
# @From : https://github.com/apple/ml-mobileone
import os.path
...
class MobileOneNet(nn.Module):
    def __init__(self, num_classes=6, inference_mode=False, variant='s0', pretrained=True,
                 pretrained_path='/home/ycc/Downloads/mobileone_s0_unfused.pth.tar'):
        super(MobileOneNet, self).__init__()
        self.num_classes = num_classes
        self.inference_mode = inference_mode
        self.variant = variant
        self.model = nn.Sequential(
            *list(mobileone(self.num_classes, self.inference_mode, self.variant).children())[:-2])

        # load fine-tune model
        if pretrained:
            print('will pretrained ModelOneNet')
            if not os.path.exists(pretrained_path):
                pass
            pretrain_model = torch.load(pretrained_path)
            pretrain_model_dict = dict(pretrain_model.items())
            self.model_dict = dict(self.model.state_dict())
            for k, v in self.model_dict.items():
                # pk = k.replace('model.', 'stage')
                pk = 'stage' + k
                self.model_dict[k] = pretrain_model_dict[pk]
            self.model.load_state_dict(self.model_dict)
            print('load pretrained MobileOneNet')
...
if __name__ == '__main__':
    ...
    model = MobileOneNet(num_classes=10, inference_mode=False, variant='s0')
    ...
    pre_model = torch.load('/home/ycc/Downloads/mobileone_s0_unfused.pth.tar')

    pre_model_dict = dict(pre_model.items())
    model_dict = dict(model.state_dict())
    for k, v in model_dict.items():
        p_v = model_dict[k].sum()
        p_k = k.replace('model.', 'stage')
    ...
    a = torch.load('/home/ycc/Downloads/mobileone_s0_unfused.pth.tar')
    print(type(a))
    # layer = 0
    ...
    print(layer)
    exit(0)
    torch.save(model.model.eval(), 'mobileone_train.pt')
    ...
    # 创建示例输入并运行模型以生成输出张量
    input_shape = (1, 3, 224, 224)  # 输入形状（batch_size, channels, height, width）
    dummy_input = torch.randn(input_shape)
    output = model(dummy_input)

    # 将模型转换为 ONNX 格式
    onnx_file_name = "mobileone_train.onnx"
    torch.onnx.export(model, dummy_input, onnx_file_name, verbose=False,
                      input_names=["input"], output_names=["output"])

    # 验证 ONNX 模型是否可以加载并生成与 PyTorch 输出相同的结果
    ...
    print(f"PyTorch output: {output[0][:5]}")
    print(f"ONNX Runtime output: {ort_outputs[0][:5]}")

    model = MobileOneNet(num_classes=10, inference_mode=True, variant='s0')
    torch.save(model.model.eval(), 'mobileone_test.pt')
    ...
    # 将模型转换为 ONNX 格式
    onnx_file_name = "mobileone_test.onnx"
    torch.onnx.export(model, dummy_input, onnx_file_name, verbose=False,
                      input_names=["input"], output_names=["output"])
