...
try:
    from collections.abc import Sequence        # python3.10
except ImportError:
    from collections import Sequence            # < 3.10
...
def to_tensor(image):
    '''
    Description: Convert image(nd.array) to Tensor.
    Args (type): image
...
def hflip(image):
    '''
    Description: Horizontally flip the given image
    Args (type):
    Return:
        Converted image
    '''
    if not _is_numpy_image(image):
        raise TypeError("image should be np.ndarray image. Got {}".format(type(image)))
    if image.ndim == 2:
        image = image[:, :, None]
...
def vflip(image):
    '''
    Description: Vertically flip the given image
    Args (type):
    Return:
        Converted image
    '''
...
def randomcrop(image, output_size):
    '''
    Description: randomcrop the given image
    Args (type):
    Return:
        Converted image
    '''
...
def rescale(image, output_size, interpolation=cv.INTER_LINEAR):
    '''
    Description: randomscale the image to a given size.
    Args (type):
...
def rescale_pad(image, output_size, interpolation=cv.INTER_LINEAR, fill=0, padding_mode='constant'):
...
def random_erasing(image, sl, sh, rl, rh):
    '''
...
def shift_padding(image, hor_shift_ratio, ver_shift_ratio):
    if not _is_numpy_image(image):
    ...
    new_image[int(np.round(ver_shift_ratio[1] * h)):int(np.round(ver_shift_ratio[1] * h)) + h,
    int(np.round(hor_shift_ratio[1] * w)):int(np.round(hor_shift_ratio[1] * w)) + w] = image
...
def randomrotation(image, degree, center=None):
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
...
def randomcropresize(image, p, output_size, scale, ratio):
    """
    Description:
...
def peppernoise(image, percentage=0.1):
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
...
def depeppernoise(image, percentage=0.1):
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
...
def gaussiannoise(image, percentage=0.1, mean=0, sigma=25):
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
...
def resize_pad(image, resize, padsize):
    h, w, c = image.shape
    if w > h:
