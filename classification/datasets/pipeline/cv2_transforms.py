...
from . import cv2_functionals as F
    ...
    def __init__(self, transforms):
        ...
        for transform_cfg in transforms:
            if isinstance(transform_cfg, dict):
                ...
                transform = PIPELINES[transform_type](**transform_cfg)
...
class IAAPerspective(object):
    def __init__(self, p=0.1, scale=(0.05, 0.15)):
        self.p = p
        self.scale = scale
        self.aug = A.Perspective(p=p, scale=scale)
...
@PIPELINES.register_module
class CLAHE(object):
    def __init__(self, clip_limit=4.0, p=0.5):
        self.p = p
        self.clip_limit = clip_limit
        self.aug = A.CLAHE(clip_limit=self.clip_limit, p=self.p)
...
@PIPELINES.register_module
class DrawHair(object):
    def __init__(self, haris=5, width=(1, 2), p=0.5):
        self.p = p
        self.hairs = haris
        self.width = width

    def __call__(self, img):
        if not self.hairs or np.random.random() <= self.p:
            return img

        width, height, _ = img.shape
        hairs_nums = random.randint(0, self.hairs)
        for _ in range(hairs_nums):
            # The origin point of the line will always be at the top half of the image
            start_pt = (random.randint(0, width), random.randint(0, height // 2))
            # The end of the line
            end_pt = (random.randint(0, width), random.randint(0, height))

            mid_pt = ((start_pt[0] + end_pt[0]) // 2, (start_pt[1] + end_pt[1]) // 2)
            dx = abs(start_pt[0] - end_pt[0]) * 0.5
            dy = abs(start_pt[1] - end_pt[1]) * 0.5
            control_pts = np.array([[mid_pt[0] - dx, mid_pt[1] + dy], [mid_pt[0] + dx, mid_pt[1] - dy]])

            if random.uniform(0, 1) < 0.5:
                color = (0, 0, 0)  # color of the hair. Black.
            else:
                color = (255, 255, 255)
            pts = np.array([start_pt, control_pts[0], control_pts[1], end_pt], np.int32)
            cv.polylines(img, [pts], False, color, random.randint(self.width[0], self.width[1]))
